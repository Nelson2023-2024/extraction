...
class AirtelStatementExtractor:
    ...
    def extract_metadata(self, page1_text):
        ...
        date_range = self.extract(
            r"(\d{2} [A-Z][a-z]{2} \d{4} to \d{2} [A-Z][a-z]{2} \d{4})", page1_text
        )
        ...
        opening_bal = self.clean_num(
            self.extract(r"Opening Balance\s*([\d,.]+)", page1_text)
        )
        closing_bal = self.clean_num(
            self.extract(r"Closing Balance\s*([\d,.]+)", page1_text)
        )
        total_credit = self.clean_num(
            self.extract(r"Total Money Credited\s*([\d,.]+)", page1_text)
        )
        total_debit = self.clean_num(
            self.extract(r"Total Money Debited\s*([\d,.]+)", page1_text)
        )
